                    ...
                    <div class="grid justify-content-between align-items-center">
                        <div  class="col " >
                            <div class=" text-2xl text-primary font-bold" >
                                {{ $t('userAuthentications') }}
                            </div>
                        </div>
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('userauthentications/add')">
                                <router-link :to="`/userauthentications/add`">
                                    <Button :label="$t('addNewUserAuthentication')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/userauthentications'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="ua_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="ua_id" :header="$t('uaId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/userauthentications/view/${data.ua_id}`">
                                                    {{ data.ua_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="auth_type" :header="$t('authType')" >
                                            <template #body="{ data, index }">
                                                {{ data.auth_type }}
                                            </template>
                                        </Column>
                                        <Column  field="auth_id" :header="$t('authId')" >
                                            <template #body="{ data, index }">
                                                {{ data.auth_id }}
                                            </template>
                                        </Column>
                                        <Column  field="auth_data" :header="$t('authData')" >
                                            <template #body="{ data, index }">
                                                {{ data.auth_data }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('userauthentications')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'userauthenticationslist',
		},
		apiPath: {
			type: String,
			default: 'userauthentications/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/userauthentications/view/${data.ua_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('userauthentications/view')
		},
		{
			label: () => $t('edit'),
			to: `/userauthentications/edit/${data.ua_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('userauthentications/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.ua_id) },
			icon: "pi pi-trash",
			visible: auth.canView('userauthentications/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('userAuthentications');
