                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/tags'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="tag_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="tag_id" :header="$t('tagId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/tags/view/${data.tag_id}`">
                                                    {{ data.tag_id }}
                                                </router-link>
                                            </template>
                                        </Column>
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/tags/view/${data.tag_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/tags/edit/${data.tag_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
