            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('contentslikes/add')">
                                <router-link :to="`/contentslikes/add`">
                                    <Button :label="$t('addNewContentsLike')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/contentslikes'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="like_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="like_id" :header="$t('likeId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/contentslikes/view/${data.like_id}`">
                                                    {{ data.like_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="liked_at" :header="$t('likedAt')" >
                                            <template #body="{ data, index }">
                                                {{$utils.humanDatetime( data.liked_at )}}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('contentslikes')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'contentslikeslist',
		},
		apiPath: {
			type: String,
			default: 'contentslikes/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/contentslikes/view/${data.like_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('contentslikes/view')
		},
		{
			label: () => $t('edit'),
			to: `/contentslikes/edit/${data.like_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('contentslikes/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.like_id) },
			icon: "pi pi-trash",
			visible: auth.canView('contentslikes/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('contentsLikes');
