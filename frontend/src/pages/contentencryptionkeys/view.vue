                                ...
                                <div class="mb-3 grid ">
                                    <div class="col-12 md:col-4">
                                        <div class="flex gap-2 align-items-center card shadow-none p-3 surface-100 ">
                                            <div class="">
                                                <div class="text-400 mb-1">{{ $t('keyId') }}</div>
                                                <div class="font-bold">{{ item.key_id }}</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-12 md:col-4">
                                    ...
                                    <div class="col-12 md:col-4">
                                        <div class="flex gap-2 align-items-center card shadow-none p-3 surface-100 ">
                                            <div class="">
                                                <div class="text-400 mb-1">{{ $t('encryptedKey') }}</div>
                                                <div class="font-bold">{{ item.encrypted_key }}</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-12 md:col-4">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'contentencryptionkeysview',
		},
		apiPath: {
			type: String,
			default: 'contentencryptionkeys/view',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('edit'),
			command: (event) => { app.openPageDialog({ page:'contentencryptionkeys/edit', url: `/contentencryptionkeys/edit/${data.key_id}`, closeBtn: true }) },
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
	...
	onMounted(()=>{ 
		const pageTitle = $t('contentEncryptionKeyDetails');
