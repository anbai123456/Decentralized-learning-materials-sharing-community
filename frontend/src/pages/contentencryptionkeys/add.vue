                                    ...
                                    <div class="grid">
                                        ...
                                        <div class="col-12">
                                            <div class="formgrid grid">
                                                ...
                                                <div class="col-12 md:col-9">
                                                    <Textarea :class="getErrorClass('encrypted_key')" class="w-full" ref="ctrlencrypted_key" rows="5"  v-model="formData.encrypted_key" :placeholder="$t('enterEncryptedKey')"    type="textarea">
                                                    </Textarea>
                                                    <small v-if="isFieldValid('encrypted_key')" class="p-error">{{ getFieldError('encrypted_key') }}</small> 
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName : {
			type : String,
			default : 'contentencryptionkeysadd',
		},
		apiPath : {
			type : String,
			default : 'contentencryptionkeys/add',
		},
		submitButtonLabel: {
			type: String,
			default: () => $t('submit'),
	...
	function afterSubmit(response) { 
		...
		if(app.isDialogOpen()){
			...
			app.navigateTo(`/contentencryptionkeys`);
		}
	}
	...
	onMounted(()=>{
		const pageTitle = $t('addNewContentEncryptionKey');
