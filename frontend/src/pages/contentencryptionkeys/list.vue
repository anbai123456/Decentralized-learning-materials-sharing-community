                    ...
                    <div class="grid justify-content-between align-items-center">
                        <div  class="col " >
                            <div class=" text-2xl text-primary font-bold" >
                                {{ $t('contentEncryptionKeys') }}
                            </div>
                        </div>
                        <div  class="col-fixed " >
                            <router-link :to="`/contentencryptionkeys/add`">
                                <Button :label="$t('addNewContentEncryptionKey')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/contentencryptionkeys'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="key_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="key_id" :header="$t('keyId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/contentencryptionkeys/view/${data.key_id}`">
                                                    {{ data.key_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="encrypted_key" :header="$t('encryptedKey')" >
                                            <template #body="{ data, index }">
                                                {{ data.encrypted_key }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'contentencryptionkeyslist',
		},
		apiPath: {
			type: String,
			default: 'contentencryptionkeys/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/contentencryptionkeys/view/${data.key_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/contentencryptionkeys/edit/${data.key_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.key_id) },
			icon: "pi pi-trash"
		}
	]
	}
	...
	onMounted(()=>{ 
		const pageTitle = $t('contentEncryptionKeys');
