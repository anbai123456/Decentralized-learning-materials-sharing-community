            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/ipfsnodes/add`">
                                <Button :label="$t('addNewIpfsNode')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/ipfsnodes'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="node_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="node_id" :header="$t('nodeId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/ipfsnodes/view/${data.node_id}`">
                                                    {{ data.node_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="node_address" :header="$t('nodeAddress')" >
                                            <template #body="{ data, index }">
                                                {{ data.node_address }}
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="last_seen" :header="$t('lastSeen')" >
                                            <template #body="{ data, index }">
                                                {{ data.last_seen }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'ipfsnodeslist',
		},
		apiPath: {
			type: String,
			default: 'ipfsnodes/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/ipfsnodes/view/${data.node_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/ipfsnodes/edit/${data.node_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.node_id) },
			icon: "pi pi-trash"
		}
	]
	}
