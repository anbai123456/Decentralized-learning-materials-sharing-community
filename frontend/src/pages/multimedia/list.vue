            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('multimedia/add')">
                                <router-link :to="`/multimedia/add`">
                                    <Button :label="$t('addNewMultimedia')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col-3 comp-grid" >
                        ...
                        <router-link  :to="`/multimedia`">
                            ...
                            <div :class="{ 'card ': !isSubPage }" class="p-3 mb-3">
                                <api-data-source :enable-cache="true" @loaded="(response)=> filters.user_id.options=response"  api-path="components_data/user_id_option_list_3" >
                                    <template v-slot="req">
                        ...
                        <div  class="col-9 comp-grid" >
                            ...
                            <div >
                                <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                    <Breadcrumb :home="{icon: 'pi pi-home', to: '/multimedia'}" :model="pageBreadCrumb" />
                                </template>
                                <!-- page records template -->
                                <div class="page-records"  >
                                    <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                         :value="records" dataKey="multimedia_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                        <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                            <Column  field="" header="" >
                                                <template #body="{ data, index }">
                                                    ...
                                                    app.openPageDialog({ page: 'multimedia/detail-pages', pageData: data , closeBtn: true }); 
                                                    }"
                                                    label="" icon="pi pi-caret-down" />
                                                </template>
                                            </Column>
                                            <Column  field="multimedia_id" :header="$t('mId')" :sortable="true">
                                                <template #body="{ data, index }">
                                                    <router-link :to="`/multimedia/view/${data.multimedia_id}`">
                                                        {{ data.multimedia_id }}
                                                    </router-link>
                                                </template>
                                            </Column>
                                            ...
                                            <Column  field="image_file" :header="$t('imageFile')" >
                                                <template #body="{ data, index }">
                                                    <image-viewer image-size="small" image-preview-size="" :src="data.image_file" width="100px" height="100px" class="img-fluid" :num-display="1">
                                                    </image-viewer>
                                                </template>
                                            </Column>
                                            <Column  field="video_file" :header="$t('videoFile')" >
                                                <template #body="{ data, index }">
                                                    <video controls width="150"> 
                                                    <source :src="$utils.getFileFullPath(data.video_file)" type="video/mp4" />
                                                    </video> 
                                                </template>
                                            </Column>
                                            <Column  field="doc_file" :header="$t('docFile')" >
                                                <template #body="{ data, index }">
                                                    <file-viewer icon="pi pi-paperclip" v-if="data.doc_file" v-model="data.doc_file"></file-viewer>
                                                </template>
                                            </Column>
                                            <Column  field="description" :header="$t('description')" >
                                                <template #body="{ data, index }">
                                                    {{$utils.truncate( data.description , 8, '...')}}
                                                </template>
                                            </Column>
                                            ...
                                            <Column  field="learntime" :header="$t('ltime')" >
                                                <template #body="{ data, index }">
                                                    {{ data.learntime }}
                                                </template>
                                            </Column>
                                    ...
                                    <template v-if="showFooter && pageReady">
                                        <div class="grid justify-content-between align-items-center">
                                            <div class="flex gap-2 flex-grow-0">
                                                <template v-if="auth.canView('multimedia')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'multimedialist',
		},
		apiPath: {
			type: String,
			default: 'multimedia/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/multimedia/view/${data.multimedia_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('multimedia/view')
		},
		{
			label: () => $t('edit'),
			to: `/multimedia/edit/${data.multimedia_id}`,
			icon: "pi pi-pencil",
