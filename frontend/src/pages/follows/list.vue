            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('follows/add')">
                                <router-link :to="`/follows/add`">
                                    <Button :label="$t('addNewFollow')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/follows'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="follows_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="follows_id" :header="$t('followsId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/follows/view/${data.follows_id}`">
                                                    {{ data.follows_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="followee_user_id" :header="$t('followeeUserId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('users')" v-if="data.followee_user_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.followee_user_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        <Column  field="follower_user_id" :header="$t('followerUserId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('users')" v-if="data.follower_user_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.follower_user_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        <Column  field="follow_time" :header="$t('followTime')" >
                                            <template #body="{ data, index }">
                                                {{ data.follow_time }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/follows/view/${data.follows_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('follows/view')
		},
		{
			label: () => $t('edit'),
			to: `/follows/edit/${data.follows_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('follows/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.follows_id) },
			icon: "pi pi-trash",
			visible: auth.canView('follows/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('follows');
