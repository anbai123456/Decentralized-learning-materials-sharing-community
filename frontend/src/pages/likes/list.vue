            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('likes/add')">
                                <router-link :to="`/likes/add`">
                                    <Button :label="$t('addNewLike')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/likes'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="like_id" :header="$t('likeId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/likes/view/${data.like_id}`">
                                                    {{ data.like_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="liked_count" :header="$t('likedCount')" >
                                            <template #body="{ data, index }">
                                                {{ data.liked_count }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/likes/view/${data.like_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('likes/view')
		},
		{
			label: () => $t('edit'),
			to: `/likes/edit/${data.like_id}`,
			icon: "pi pi-pencil",
