            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('cstart/add')">
                                <router-link :to="`/cstart/add`">
                                    <Button :label="$t('addNewCStart')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/cstart'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="start_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="start_id" :header="$t('startId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/cstart/view/${data.start_id}`">
                                                    {{ data.start_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="user_id" :header="$t('userId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="data.users_user_id" v-if="data.user_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.user_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        <Column  field="starteduser_id" :header="$t('starteduserId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('users')" v-if="data.starteduser_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.starteduser_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        <Column  field="start_at" :header="$t('startAt')" >
                                            <template #body="{ data, index }">
                                                {{ data.start_at }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/cstart/view/${data.start_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('cstart/view')
		},
		{
			label: () => $t('edit'),
			to: `/cstart/edit/${data.start_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('cstart/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.start_id) },
			icon: "pi pi-trash",
			visible: auth.canView('cstart/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('cStart');
