            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('ccollect/add')">
                                <router-link :to="`/ccollect/add`">
                                    <Button :label="$t('addNewCCollect')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/ccollect'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="collect_id" :header="$t('collectId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/ccollect/view/${data.collect_id}`">
                                                    {{ data.collect_id }}
                                                </router-link>
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'ccollectlist',
		},
		apiPath: {
			type: String,
			default: 'ccollect/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/ccollect/view/${data.collect_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('ccollect/view')
		},
		{
			label: () => $t('edit'),
			to: `/ccollect/edit/${data.collect_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('ccollect/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.collect_id) },
			icon: "pi pi-trash",
			visible: auth.canView('ccollect/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('cCollect');
