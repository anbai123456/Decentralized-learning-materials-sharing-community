            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/usersettings/add`">
                                <Button :label="$t('addNewUserSetting')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/usersettings'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="setting_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="setting_id" :header="$t('settingId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/usersettings/view/${data.setting_id}`">
                                                    {{ data.setting_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="setting_key" :header="$t('settingKey')" >
                                            <template #body="{ data, index }">
                                                {{ data.setting_key }}
                                            </template>
                                        </Column>
                                        <Column  field="setting_value" :header="$t('settingValue')" >
                                            <template #body="{ data, index }">
                                                {{ data.setting_value }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'usersettingslist',
		},
		apiPath: {
			type: String,
			default: 'usersettings/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/usersettings/view/${data.setting_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/usersettings/edit/${data.setting_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.setting_id) },
			icon: "pi pi-trash"
		}
	]
	}
	...
	onMounted(()=>{ 
		const pageTitle = $t('userSettings');
