            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/userwallets/add`">
                                <Button :label="$t('addNewUserWallet')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/userwallets'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="wallet_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="wallet_id" :header="$t('walletId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/userwallets/view/${data.wallet_id}`">
                                                    {{ data.wallet_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="wallet_address" :header="$t('walletAddress')" >
                                            <template #body="{ data, index }">
                                                {{ data.wallet_address }}
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="last_updated" :header="$t('lastUpdated')" >
                                            <template #body="{ data, index }">
                                                {{ data.last_updated }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'userwalletslist',
		},
		apiPath: {
			type: String,
			default: 'userwallets/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/userwallets/view/${data.wallet_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/userwallets/edit/${data.wallet_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.wallet_id) },
			icon: "pi pi-trash"
		}
	]
	}
