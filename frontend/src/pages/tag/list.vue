            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('tag/add')">
                                <router-link :to="`/tag/add`">
                                    <Button :label="$t('addNewTag')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col-3 comp-grid" >
                        <div class="">
                            <div class="card my-3 p-3 surface-50">
                                <TagAddPage ref="tagAddPage"  :show-header="true" is-sub-page>
                                </TagAddPage>
                            </div>
                        </div>
                    </div>
                    <div  class="col-9 comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/tag'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="id" :header="$t('id')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/tag/view/${data.id}`">
                                                    {{ data.id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('tag')">
...
<script setup>
	...
	import TagAddPage from "../tag/add.vue";
	...
	function getActionMenuModel(data){
		return [
		{
			...
			to: `/tag/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('tag/view')
		},
		{
			label: () => $t('edit'),
			to: `/tag/edit/${data.id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('tag/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: auth.canView('tag/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('tag');
