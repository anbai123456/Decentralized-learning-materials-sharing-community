                    ...
                    <div class="grid justify-content-between align-items-center">
                        <div  class="col " >
                            <div class=" text-2xl text-primary font-bold" >
                                {{ $t('contentsReplyReply') }}
                            </div>
                        </div>
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('contentsreplyreply/add')">
                                <router-link :to="`/contentsreplyreply/add`">
                                    <Button :label="$t('addNewContentsReplyReply')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/contentsreplyreply'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="reply_reply_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="reply_reply_id" :header="$t('replyReplyId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/contentsreplyreply/view/${data.reply_reply_id}`">
                                                    {{ data.reply_reply_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="reply_reply_content" :header="$t('replyReplyContent')" >
                                            <template #body="{ data, index }">
                                                {{ data.reply_reply_content }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('contentsreplyreply')">
...
<script setup>
	...
	const props = defineProps({
		primaryKey : {
			type : String,
			default : 'reply_reply_id',
		},
		pageStoreKey: {
			type: String,
			default: 'CONTENTSREPLYREPLY',
		},
		pageName: {
			type: String,
			default : 'contentsreplyreply',
		...
		routeName: {
			type: String,
			default: 'contentsreplyreplylist',
		},
		apiPath: {
			type: String,
			default: 'contentsreplyreply/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/contentsreplyreply/view/${data.reply_reply_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('contentsreplyreply/view')
		},
		{
			label: () => $t('edit'),
			to: `/contentsreplyreply/edit/${data.reply_reply_id}`,
			icon: "pi pi-pencil",
			visible: auth.canManage('contentsreplyreply/edit', data.user_id)
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.reply_reply_id) },
			icon: "pi pi-trash",
			visible: auth.canManage('contentsreplyreply/delete', data.user_id)
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('contentsReplyReply');
