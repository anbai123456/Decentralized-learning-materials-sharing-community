            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/groupmembers/add`">
                                <Button :label="$t('addNewGroupMember')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/groupmembers'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="group_id" :header="$t('groupId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('userGroups')" v-if="data.group_id" @click="app.openPageDialog({ page: 'usergroups/view', url: `/usergroups/view/${data.group_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="joined_at" :header="$t('joinedAt')" >
                                            <template #body="{ data, index }">
                                                {{ data.joined_at }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'groupmemberslist',
		},
		apiPath: {
			type: String,
			default: 'groupmembers/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/groupmembers/view/${data.user_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/groupmembers/edit/${data.user_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
	...
	onMounted(()=>{ 
		const pageTitle = $t('groupMembers');
