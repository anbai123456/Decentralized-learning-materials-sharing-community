                    ...
                    <div class="grid justify-content-between align-items-center">
                        <div  class="col " >
                            <div class=" text-2xl text-primary font-bold" >
                                {{ $t('contentCategoryRelations') }}
                            </div>
                        </div>
                        <div  class="col-fixed " >
                            <router-link :to="`/contentcategoryrelations/add`">
                                <Button :label="$t('addNewContentCategoryRelation')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/contentcategoryrelations'}" :model="pageBreadCrumb" />
                            </template>
                                ...
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        ...
                                        <Column  field="category_id" :header="$t('categoryId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('contentCategories')" v-if="data.category_id" @click="app.openPageDialog({ page: 'contentcategories/view', url: `/contentcategories/view/${data.category_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		pageStoreKey: {
			type: String,
			default: 'CONTENTCATEGORYRELATIONS',
		},
		pageName: {
			type: String,
			default : 'contentcategoryrelations',
		...
		routeName: {
			type: String,
			default: 'contentcategoryrelationslist',
		},
		apiPath: {
			type: String,
			default: 'contentcategoryrelations/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/contentcategoryrelations/view/${data.category_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/contentcategoryrelations/edit/${data.category_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
	...
	onMounted(()=>{ 
		const pageTitle = $t('contentCategoryRelations');
