            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('fans/add')">
                                <router-link :to="`/fans/add`">
                                    <Button :label="$t('addNewFan')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/fans'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="fans_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="fans_id" :header="$t('fansId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/fans/view/${data.fans_id}`">
                                                    {{ data.fans_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="ismutualfollow" :header="$t('ismutualfollow')" >
                                            <template #body="{ data, index }">
                                                {{ data.ismutualfollow }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/fans/view/${data.fans_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('fans/view')
		},
		{
			label: () => $t('edit'),
			to: `/fans/edit/${data.fans_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('fans/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.fans_id) },
			icon: "pi pi-trash",
			visible: auth.canView('fans/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('fans');
