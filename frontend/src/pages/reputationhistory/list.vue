            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/reputationhistory/add`">
                                <Button :label="$t('addNewReputationHistory')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/reputationhistory'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="history_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="history_id" :header="$t('historyId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/reputationhistory/view/${data.history_id}`">
                                                    {{ data.history_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="change_amount" :header="$t('changeAmount')" >
                                            <template #body="{ data, index }">
                                                {{ data.change_amount }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'reputationhistorylist',
		},
		apiPath: {
			type: String,
			default: 'reputationhistory/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/reputationhistory/view/${data.history_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/reputationhistory/edit/${data.history_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
	...
	onMounted(()=>{ 
		const pageTitle = $t('reputationHistory');
