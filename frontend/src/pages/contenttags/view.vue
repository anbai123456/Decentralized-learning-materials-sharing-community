                                    ...
                                    <div class="col-12 md:col-4">
                                        <div class="flex gap-2 align-items-center card shadow-none p-3 surface-100 ">
                                            <div class="">
                                                ...
                                                <div class="font-bold">
                                                    <Button class="p-button-text" icon="pi pi-eye" :label="$t('tagsDetail')" v-if="item.tag_id" @click="app.openPageDialog({ page: 'tags/view', url: `/tags/view/${item.tag_id}` , closeBtn: true })" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'contenttagsview',
		},
		apiPath: {
			type: String,
			default: 'contenttags/view',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('edit'),
			command: (event) => { app.openPageDialog({ page:'contenttags/edit', url: `/contenttags/edit/${data.tag_id}`, closeBtn: true }) },
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
	...
	onMounted(()=>{ 
		const pageTitle = $t('contentTagDetails');
