            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/contenttags/add`">
                                <Button :label="$t('addNewContentTag')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/contenttags'}" :model="pageBreadCrumb" />
                            </template>
                                ...
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        ...
                                        <Column  field="tag_id" :header="$t('tagId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('tags')" v-if="data.tag_id" @click="app.openPageDialog({ page: 'tags/view', url: `/tags/view/${data.tag_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'contenttagslist',
		},
		apiPath: {
			type: String,
			default: 'contenttags/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/contenttags/view/${data.tag_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/contenttags/edit/${data.tag_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
