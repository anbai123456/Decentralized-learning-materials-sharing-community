                            ...
                            <div >
                                <div class="mb-3 grid ">
                                    <div class="col-12 md:col-4">
                                        <div class="flex gap-2 align-items-center card shadow-none p-3 surface-100 ">
                                            <div class="">
                                                ...
                                                <div class="font-bold">{{ item.group_id }}</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-12 md:col-4">
                                    ...
                                    <div class="col-12 md:col-4">
                                        <div class="flex gap-2 align-items-center card shadow-none p-3 surface-100 ">
                                            <div class="">
                                                <div class="text-400 mb-1">{{ $t('creatorId') }}</div>
                                                <div class="font-bold">
                                                    <Button class="p-button-text" icon="pi pi-eye" :label="$t('usersDetail')" v-if="item.creator_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${item.creator_id}` , closeBtn: true })" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'usergroupsview',
		},
		apiPath: {
			type: String,
			default: 'usergroups/view',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('edit'),
			command: (event) => { app.openPageDialog({ page:'usergroups/edit', url: `/usergroups/edit/${data.group_id}`, closeBtn: true }) },
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
	...
	onMounted(()=>{ 
		const pageTitle = $t('userGroupDetails');
