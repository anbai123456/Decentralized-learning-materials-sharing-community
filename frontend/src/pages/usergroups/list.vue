            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/usergroups/add`">
                                <Button :label="$t('addNewUserGroup')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/usergroups'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="group_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="group_id" :header="$t('groupId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/usergroups/view/${data.group_id}`">
                                                    {{ data.group_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="creator_id" :header="$t('creatorId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('users')" v-if="data.creator_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.creator_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'usergroupslist',
		},
		apiPath: {
			type: String,
			default: 'usergroups/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/usergroups/view/${data.group_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/usergroups/edit/${data.group_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.group_id) },
			icon: "pi pi-trash"
		}
	]
	}
