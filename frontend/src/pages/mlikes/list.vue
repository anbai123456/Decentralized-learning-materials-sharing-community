            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('mlikes/add')">
                                <router-link :to="`/mlikes/add`">
                                    <Button :label="$t('addNewMLike')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/mlikes'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="like_id" :header="$t('likeId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/mlikes/view/${data.like_id}`">
                                                    {{ data.like_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('mlikes')">
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/mlikes/view/${data.like_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('mlikes/view')
		},
		{
			label: () => $t('edit'),
			to: `/mlikes/edit/${data.like_id}`,
			icon: "pi pi-pencil",
