                    ...
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/transactions/add`">
                                <Button :label="$t('addNewTransaction')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                        ...
                        <div >
                            ...
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="transaction_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="transaction_id" :header="$t('transactionId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/transactions/view/${data.transaction_id}`">
                                                    {{ data.transaction_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="from_user_id" :header="$t('fromUserId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('users')" v-if="data.from_user_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.from_user_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        <Column  field="to_user_id" :header="$t('toUserId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('users')" v-if="data.to_user_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.to_user_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="transaction_type" :header="$t('transactionType')" >
                                            <template #body="{ data, index }">
                                        ...
                                        <Column  field="blockchain_tx" :header="$t('blockchainTx')" >
                                            <template #body="{ data, index }">
                                                {{ data.blockchain_tx }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/transactions/view/${data.transaction_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/transactions/edit/${data.transaction_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.transaction_id) },
			icon: "pi pi-trash"
		}
	]
	}
