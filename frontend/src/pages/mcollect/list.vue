            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('mcollect/add')">
                                <router-link :to="`/mcollect/add`">
                                    <Button :label="$t('addNewMCollect')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/mcollect'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="collect_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="collect_id" :header="$t('collectId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/mcollect/view/${data.collect_id}`">
                                                    {{ data.collect_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="multimedia_id" :header="$t('multimediaId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('multimedia')" v-if="data.multimedia_id" @click="app.openPageDialog({ page: 'multimedia/view', url: `/multimedia/view/${data.multimedia_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        <Column  field="collect_at" :header="$t('collectAt')" >
                                            <template #body="{ data, index }">
                                                {{ data.collect_at }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('mcollect')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'mcollectlist',
		},
		apiPath: {
			type: String,
			default: 'mcollect/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/mcollect/view/${data.collect_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('mcollect/view')
		},
		{
			label: () => $t('edit'),
			to: `/mcollect/edit/${data.collect_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('mcollect/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.collect_id) },
			icon: "pi pi-trash",
			visible: auth.canView('mcollect/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('mCollect');
