            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/reports/add`">
                                <Button :label="$t('addNewReport')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/reports'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="report_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="report_id" :header="$t('reportId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/reports/view/${data.report_id}`">
                                                    {{ data.report_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="reporter_id" :header="$t('reporterId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('users')" v-if="data.reporter_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.reporter_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        <Column  field="reported_content_id" :header="$t('reportedContentId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('contents')" v-if="data.reported_content_id" @click="app.openPageDialog({ page: 'contents/view', url: `/contents/view/${data.reported_content_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/reports/view/${data.report_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/reports/edit/${data.report_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
