            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/systemconfigs/add`">
                                <Button :label="$t('addNewSystemConfig')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/systemconfigs'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="config_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="config_id" :header="$t('configId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/systemconfigs/view/${data.config_id}`">
                                                    {{ data.config_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="config_key" :header="$t('configKey')" >
                                            <template #body="{ data, index }">
                                                {{ data.config_key }}
                                            </template>
                                        </Column>
                                        <Column  field="config_value" :header="$t('configValue')" >
                                            <template #body="{ data, index }">
                                                {{ data.config_value }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'systemconfigslist',
		},
		apiPath: {
			type: String,
			default: 'systemconfigs/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/systemconfigs/view/${data.config_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/systemconfigs/edit/${data.config_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.config_id) },
			icon: "pi pi-trash"
		}
	]
	}
	...
	onMounted(()=>{ 
		const pageTitle = $t('systemConfigs');
