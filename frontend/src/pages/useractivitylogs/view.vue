                            ...
                            <div >
                                <div class="mb-3 grid ">
                                    ...
                                    <div class="col-12 md:col-4">
                                        <div class="flex gap-2 align-items-center card shadow-none p-3 surface-100 ">
                                            <div class="">
                                                <div class="text-400 mb-1">{{ $t('activityType') }}</div>
                                                <div class="font-bold">{{ item.activity_type }}</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-12 md:col-4">
                                        <div class="flex gap-2 align-items-center card shadow-none p-3 surface-100 ">
                                            <div class="">
                                                <div class="text-400 mb-1">{{ $t('activityData') }}</div>
                                                <div class="font-bold">{{ item.activity_data }}</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-12 md:col-4">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'useractivitylogsview',
		},
		apiPath: {
			type: String,
			default: 'useractivitylogs/view',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('edit'),
			command: (event) => { app.openPageDialog({ page:'useractivitylogs/edit', url: `/useractivitylogs/edit/${data.log_id}`, closeBtn: true }) },
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
	...
	onMounted(()=>{ 
		const pageTitle = $t('userActivityLogDetails');
