            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/useractivitylogs/add`">
                                <Button :label="$t('addNewUserActivityLog')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/useractivitylogs'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="log_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="log_id" :header="$t('logId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/useractivitylogs/view/${data.log_id}`">
                                                    {{ data.log_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="activity_type" :header="$t('activityType')" >
                                            <template #body="{ data, index }">
                                                {{ data.activity_type }}
                                            </template>
                                        </Column>
                                        <Column  field="activity_data" :header="$t('activityData')" >
                                            <template #body="{ data, index }">
                                                {{ data.activity_data }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'useractivitylogslist',
		},
		apiPath: {
			type: String,
			default: 'useractivitylogs/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/useractivitylogs/view/${data.log_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/useractivitylogs/edit/${data.log_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.log_id) },
			icon: "pi pi-trash"
		}
	]
	}
	...
	onMounted(()=>{ 
		const pageTitle = $t('userActivityLogs');
