                    ...
                    <div class="grid justify-content-between align-items-center">
                        <div  class="col " >
                            <div class=" text-2xl text-primary font-bold" >
                                {{ $t('smartContractEvents') }}
                            </div>
                        </div>
                        <div  class="col-fixed " >
                            <router-link :to="`/smartcontractevents/add`">
                                <Button :label="$t('addNewSmartContractEvent')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/smartcontractevents'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="event_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="event_id" :header="$t('eventId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/smartcontractevents/view/${data.event_id}`">
                                                    {{ data.event_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="contract_address" :header="$t('contractAddress')" >
                                            <template #body="{ data, index }">
                                                {{ data.contract_address }}
                                            </template>
                                        </Column>
                                        <Column  field="event_name" :header="$t('eventName')" >
                                            <template #body="{ data, index }">
                                                {{ data.event_name }}
                                            </template>
                                        </Column>
                                        <Column  field="event_data" :header="$t('eventData')" >
                                            <template #body="{ data, index }">
                                                {{ data.event_data }}
                                            </template>
                                        </Column>
                                        <Column  field="block_number" :header="$t('blockNumber')" >
                                            <template #body="{ data, index }">
                                                {{ data.block_number }}
                                            </template>
                                        </Column>
                                        <Column  field="transaction_hash" :header="$t('transactionHash')" >
                                            <template #body="{ data, index }">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'smartcontracteventslist',
		},
		apiPath: {
			type: String,
			default: 'smartcontractevents/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/smartcontractevents/view/${data.event_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/smartcontractevents/edit/${data.event_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.event_id) },
			icon: "pi pi-trash"
		}
	]
	}
	...
	onMounted(()=>{ 
		const pageTitle = $t('smartContractEvents');
