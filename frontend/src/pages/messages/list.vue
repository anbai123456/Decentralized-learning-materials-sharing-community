                    ...
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/messages/add`">
                                <Button :label="$t('addNewMessage')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                        ...
                        <div >
                            ...
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="message_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="message_id" :header="$t('messageId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/messages/view/${data.message_id}`">
                                                    {{ data.message_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="sender_id" :header="$t('senderId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('users')" v-if="data.sender_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.sender_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        <Column  field="receiver_id" :header="$t('receiverId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('users')" v-if="data.receiver_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.receiver_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        <Column  field="encrypted_content" :header="$t('encryptedContent')" >
                                            <template #body="{ data, index }">
                                                {{ data.encrypted_content }}
                                            </template>
                                        </Column>
                                        <Column  field="ipfs_hash" :header="$t('ipfsHash')" >
                                            <template #body="{ data, index }">
                                                {{ data.ipfs_hash }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/messages/view/${data.message_id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/messages/edit/${data.message_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.message_id) },
			icon: "pi pi-trash"
		}
	]
	}
