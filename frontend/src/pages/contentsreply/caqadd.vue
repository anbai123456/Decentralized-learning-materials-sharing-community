                                    ...
                                    <div class="grid">
                                        ...
                                        <div class="col-12">
                                            <div class="formgrid grid">
                                                ...
                                                <div class="col-12 md:col-9">
                                                    <InputText  ref="ctrlreply_content" v-model.trim="formData.reply_content"  :label="$t('replyContent')" type="text" :placeholder="$t('enterReplyContent')"      
                                                    class=" w-full" :class="getErrorClass('reply_content')">
                                                    </InputText>
                                                    <small v-if="isFieldValid('reply_content')" class="p-error">{{ getFieldError('reply_content') }}</small> 
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName : {
			type : String,
			default : 'contentsreplycaqadd',
		},
		apiPath : {
			type : String,
			default : 'contentsreply/caqadd',
		},
		submitButtonLabel: {
			type: String,
			default: () => $t('submit'),
		...
		pageData: { // use to set formData default values from another page
			type: Object,
			default: () => ({
				content_id: "", user_id: "", reply_content: "", 
			})
		},
	});
	//lines
	...
	onMounted(()=>{
		const pageTitle = $t('addNewContentsReply');
		app.setPageTitle(props.routeName, pageTitle); // set browser page title
			formData.content_id = content_id;
	});
	 const route = useRoute() ;
 const content_id = route.query.content_id; console.log("caqadd page custom js===content_id:", content_id)
