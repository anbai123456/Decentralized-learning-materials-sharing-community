            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('contentsreply/add')">
                                <router-link :to="`/contentsreply/add`">
                                    <Button :label="$t('addNewContentsReply')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/contentsreply'}" :model="pageBreadCrumb" />
                            </template>
                            <div class="grid ">
                                <div class="col">
                                    ...
                                    <div class="page-records"  >
                                        <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                            ...
                                            <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                                ...
                                                <Column  field="reply_id" :header="$t('replyId')" :sortable="true">
                                                    <template #body="{ data, index }">
                                                        <router-link :to="`/contentsreply/view/${data.reply_id}`">
                                                            {{ data.reply_id }}
                                                        </router-link>
                                                    </template>
                                                </Column>
                                                <Column  field="content_id" :header="$t('contentId')" :sortable="true">
                                                    <template #body="{ data, index }">
                                                        {{ data.content_id }}
                                                    </template>
                                                </Column>
                                                ...
                                                <Column  field="reply_content" :header="$t('replyContent')" >
                                                    <template #body="{ data, index }">
                                                        {{ data.reply_content }}
                                                    </template>
                                                </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'contentsreplylist',
		},
		apiPath: {
			type: String,
			default: 'contentsreply/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/contentsreply/view/${data.reply_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('contentsreply/view')
		},
		{
			label: () => $t('edit'),
			to: `/contentsreply/edit/${data.reply_id}`,
			icon: "pi pi-pencil",
	...
	onMounted(()=>{ 
		const pageTitle = $t('contentsReply');
